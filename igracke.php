...
<div class="igracke-container">
      <?php 
        ...
        $sql = "SELECT * FROM igračke ORDER BY id DESC";
        $res = mysqli_query($conn, $sql );

        if(mysqli_num_rows($res) > 0){
            while ($igračke = mysqli_fetch_assoc($res)){ ?>
             
            

                    <!--igracke-1-->
              <div class="igracke-box">
                  <form method="post" action="dodaj-u-korpu.php">
                  <input type="hidden" name="email" value="<?php echo $email; ?>">
                  <input type="hidden" name="proizvod" value="<?php echo $igračke['naziv']; ?>">
                  <input type="hidden" name="cijena" value="<?php echo $igračke['cijena']; ?>">
                <div class="igracke-img">
                  <img src="slike/<?php echo $igračke['slika']?>" alt="Blog">
                </div>
                ...
                <div class="igracke-text">
                  ...
                  <button type="submit" name="dodajUKorpu" class="dodaj-btn-igracke">Додај у корпу</button>
                </div>
              </div>
